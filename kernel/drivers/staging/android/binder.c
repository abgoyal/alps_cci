...
static int binder_may_upgrade_cgrp(struct task_struct *p)
{
	...
	/* cgroup mutex MUST be held. */
	BUG_ON(!cgroup_lock_is_held());
...
{
	/* the cgroup and RT promotions should be treated as an atomic,
	 * or it's still possible to meet the tragedy. The device will hang
	 * if RT thread stays in /bg cgroup, whose rt_runtime budget is 0.
	 *
	 * synced by linux native lock of cgroup mutex.
	 */
	BUG_ON(!cgroup_lock_is_held());
