...
#define DEF_FREQUENCY_DOWN_DIFFERENTIAL     (20)  // up_threshold - down_differential
#define DEF_FREQUENCY_OD_THRESHOLD          (80)  // od 1.2G threshold
#define DEF_FREQUENCY_OD_DOWN_DIFFERENTIAL  (20)  // od 1.2G down_differential
#define DEF_FREQUENCY_UP_THRESHOLD          (40)  // up_threshold
#define DEF_SAMPLING_DOWN_FACTOR            (1)
#define MAX_SAMPLING_DOWN_FACTOR            (100000)
#define MICRO_FREQUENCY_DOWN_DIFFERENTIAL   (20)  // down_differential
#define MIN_FREQUENCY_DOWN_DIFFERENTIAL     (5)   
#define MAX_FREQUENCY_DOWN_DIFFERENTIAL     (30)
#define MICRO_FREQUENCY_UP_THRESHOLD        (40)  // up_threshold
...
static struct dbs_tuners {
    ...
    unsigned int od_down_differential;
    ...
    .od_down_differential = DEF_FREQUENCY_OD_DOWN_DIFFERENTIAL,
...
show_one(od_down_differential, od_down_differential);
...
show_one(cpu_od_count, cpu_od_count);
...
static ssize_t show_cpu_force_up(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", num_online_cpus());
}
...
static ssize_t store_od_down_differential(struct kobject *a, struct attribute *b,
                const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	dbs_tuners_ins.od_down_differential = input;
	return count;
}
...
static ssize_t store_cpu_od_count(struct kobject *a, struct attribute *b,
				    const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	dbs_tuners_ins.cpu_od_count = input;
	return count;
}
...
static ssize_t store_cpu_force_up(struct kobject *a, struct attribute *b,
				    const char *buf, size_t count)
{
	unsigned int input;
	bool raise_freq = false;
...
define_one_global_rw(od_down_differential);
...
define_one_global_rw(cpu_od_count);
...
define_one_global_rw(cpu_force_up);
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
  ...
  unsigned int freq_next;
    ...
    if (policy->cur >= get_normal_max_freq()){  // od case
        if ((max_load_freq > dbs_tuners_ins.od_threshold * policy->cur) && (num_online_cpus() == num_possible_cpus())){
            ...
            #endif
            if(g_max_cpu_persist_count == dbs_tuners_ins.cpu_od_count){
        ...
        else if(max_load_freq < (dbs_tuners_ins.od_threshold - dbs_tuners_ins.od_down_differential) * policy->cur) {
            ...
            freq_next = max_load_freq /(dbs_tuners_ins.od_threshold - dbs_tuners_ins.od_down_differential);
            ...
            if (!dbs_tuners_ins.powersave_bias) {
                ...
                int freq = powersave_bias_target(policy, freq_next, CPUFREQ_RELATION_L);
        ...
        else if (max_load_freq < (dbs_tuners_ins.up_threshold - dbs_tuners_ins.down_differential) * policy->cur) {
      	    /* Check for frequency decrease */
            /* if we cannot reduce the frequency anymore, break out early */
            if (policy->cur == policy->min)
                goto hp_check;
