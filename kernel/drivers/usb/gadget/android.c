...
void android_enable_function(struct usb_function *f, int enable)
{
    ...
    int extra = !strcmp(f->name, "adb");

#else
    int extra = 0;
#endif

	printk("[USB]%s %s\n",(enable==1?"Enable":"Disable"),f->name);
...
#ifdef USB_CDC_ACM_NORMAL_BOOT
			acm->disabled = 0; /* enable acm */
#endif

			if (!strcmp(f->name, "acm")) {
				/* when switch to ACM, do nothing.*/
		...
		{   /*update interface count*/
			...
			list_for_each_entry(func, &android_config_driver.functions, list) {
				if (func->disabled)
					...
					if (!strcmp(func->name, "rndis") || !strcmp(func->name, "acm")) {
						//rndis and acm uses 2 interfaces. So add one more.
						intf_count++;
					}
				} else
