...
void DSI_PHY_clk_setting(LCM_PARAMS *lcm_params)
{
    ...
    int pll_index = 0;
    DSI_PLL_CONFIG pll_config_value = {0, 0, 0, 0, 1, 0x01B1, 0x066D, 0x066D};

    if (lcm_params->type == LCM_TYPE_DPI)
        pll_index = custom_pll_clock_remap(lcm_params->dpi.PLL_CLOCK);
    ...
    if (pll_index < 0)
    {
        if (lcm_params->type == LCM_TYPE_DPI)
        {
            pll_config_value.TXDIV0 = lcm_params->dpi.mipi_pll_clk_div1;
            pll_config_value.TXDIV1 = lcm_params->dpi.mipi_pll_clk_div2;
            pll_config_value.SDM_PCW = lcm_params->dpi.mipi_pll_clk_fbk_div << 26;
        }
        if (lcm_params->type == LCM_TYPE_DSI)
        {
            pll_config_value.TXDIV0 = lcm_params->dsi.pll_div1;
            pll_config_value.TXDIV1 = lcm_params->dsi.pll_div2;
            pll_config_value.SDM_PCW = lcm_params->dsi.fbk_div << 26;
        }
    }
    else
    {
        memcpy(&pll_config_value, &pll_config[pll_index], sizeof(DSI_PLL_CONFIG));
    }
    ...
    OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_SSC_PRD,pll_config_value.SSC_PRD);  // fixed
    ...
    // 500MHz
    OUTREG32(&DSI_PHY_REG->MIPITX_DSI_PLL_CON2,pll_config_value.SDM_PCW);

    OUTREGBIT(MIPITX_DSI_PLL_CON3_REG,DSI_PHY_REG->MIPITX_DSI_PLL_CON3,RG_DSI0_MPPLL_SDM_SSC_DELTA,pll_config_value.SSC_DELTA);
    OUTREGBIT(MIPITX_DSI_PLL_CON3_REG,DSI_PHY_REG->MIPITX_DSI_PLL_CON3,RG_DSI0_MPPLL_SDM_SSC_DELTA1,pll_config_value.SSC_DELTA1);
...
void DSI_PHY_TIMCONFIG(LCM_PARAMS *lcm_params)
{
    ...
    {
        div1 = pll_config[pll_index].TXDIV0;
        div2 = pll_config[pll_index].TXDIV1;
        fbk_div = pll_config[pll_index].SDM_PCW >> 26;
    }
