...
static char M4U_HELP[] =
    ...
    "             enable mmprofile\n"
    "        log:[0|1|2]\n"
    "             0: disable more debug log\n"    
    "             1: enable more debug log\n"    
    "             2: disable allocate/free mva log\n"    
    "        monitor:[0|1]\n"
    "             stop/start monitor\n"     
    "        cache:[0|1]\n"
...
#ifdef M4U_MEM_EAT	
    else if (0 == strncmp(opt, "eat_mem:", 8))
    {
        char *p = (char *)opt + 8;
        unsigned int eater_buffer_size = (unsigned int) simple_strtoul(p, &p, 10);

        eater_buffer = vmalloc(eater_buffer_size * 1024 * 1024);
        if (!eater_buffer)		
        {
			M4UERR("vmalloc failed\n");
        }  
    }
#endif	
    else if (0 == strncmp(opt, "dump_smi", 8))
    {
        dump_smi_register();  
    }
    else
                      ...
                      unsigned int *pRetMVABuf)
{
    ...
    {
        //M4UMSG("Error: m4u_get_pages failed");
        M4UMSG("alloc_mva error in m4u_fill_pagetable: id=%s, addr=0x%08x, size=%d\n", m4u_get_module_name(eModuleID), BufAddr, BufSize);
    ...
    *pRetMVABuf = mvaStart;
    ...
    {
        if(true == gM4uMVALog && M4U_CLNTMOD_MDP != eModuleID)
        {
