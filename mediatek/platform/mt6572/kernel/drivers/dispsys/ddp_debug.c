...
unsigned char ddp_debug_flag=0;
...
static void process_dbg_opt(const char *opt)
{
    ...
    else if (0 == strncmp(opt, "ovl_reg:", 8))
    {
        ...
        DISP_REG_SET(DISPSYS_OVL_BASE+offset, value);
        disp_path_release_mutex_(gMutexID);

        DDP_DRV_INFO("ovl_reg: 0x%08X = 0x%08X\n", DISPSYS_OVL_BASE+offset, value);
    }
    else if (0 == strncmp(opt, "rdma_reg:", 9))
    {
        ...
        disp_path_get_mutex_(gMutexID);
        DISP_REG_SET(DISPSYS_RDMA0_BASE+offset, value);
        disp_path_release_mutex_(gMutexID);

        DDP_DRV_INFO("rdma_reg: 0x%08X = 0x%08X\n", DISPSYS_RDMA0_BASE+offset, value);
    }
    else if (0 == strncmp(opt, "bls_reg:", 8))
    {
        ...
        disp_path_get_mutex_(gMutexID);
        DISP_REG_SET(DISPSYS_BLS_BASE+offset, value);
        disp_path_release_mutex_(gMutexID);

        DDP_DRV_INFO("bls_reg: 0x%08X = 0x%08X\n", DISPSYS_BLS_BASE+offset, value);
    }
    else if (0 == strncmp(opt, "color_reg:", 10))
    {
        char *p = (char *)opt + 10;
        unsigned int offset = (unsigned int) simple_strtoul(p, &p, 16);
        unsigned int value = (unsigned int) simple_strtoul(p + 1, &p, 16);
        ...
        disp_path_get_mutex_(gMutexID);
        DISP_REG_SET(DISPSYS_COLOR_BASE+offset, value);
        disp_path_release_mutex_(gMutexID);

        DDP_DRV_INFO("color_reg: 0x%08X = 0x%08X\n", DISPSYS_COLOR_BASE+offset, value);
    }
    else
    {
	    goto Error;
