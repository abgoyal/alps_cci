...
_mali_osk_errcode_t _ump_osk_mem_mapregion_map( ump_memory_allocation * descriptor, u32 offset, u32 * phys_addr, unsigned long size )
{
    ...
    /* {MTK add
       Replace remap_pfn_range to avoid M4U error
    }*/
    retval = vm_insert_page(vma, ((u32)descriptor->mapping) + offset, phys_to_page(*phys_addr)) ? _MALI_OSK_ERR_FAULT : _MALI_OSK_ERR_OK;;
