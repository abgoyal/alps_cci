...
#define DBGOSSAR    (APARM_BASE+0x0304)
...
static int enable_hw_watchpoint(void)
{
    ...
    for(i = 0 ; i < NUM_CPU; i++) {
        if (*(volatile unsigned int *)(DBGDSCR + i * 0x2000) & HDBGEN) {
            printk(KERN_ALERT "[HWWP] Core%d halting debug mode enabled. Unable to access hardware resources.\n",i);
            return -EPERM;
        }

        if (*(volatile unsigned int *)(DBGDSCR + i * 0x2000) & MDBGEN) {
            /* already enabled */
            printk(KERN_ALERT "[HWWP] Core%d monitor debug mode already set\n", i);
        }
...
static int __init hw_watchpoint_init(void)
{
    ...
    printk(KERN_ALERT "watchpoint handler init done. \n");
