...
.PHONY : new newall remake remakeall clean cleanall \
         ...
         update-api modem-info clean-modem
...
LOGO_IMAGE    := $(LK_WD)/build-$(PROJECT)/logo.bin
...
mm:
	...
	$(hide) (source build/envsetup.sh;cd $(MM_PATH);TARGET_PRODUCT=$(TARGET_PRODUCT) FLAVOR=$(FLAVOR) mm $(MAKEJOBS) $(SNOD) $(DEAL_STDOUT_MM)) && \
...
ifeq ($(MTK_DEPENDENCY_AUTO_CHECK), true)
	...
	$(hide) mediatek/dct/DrvGen mediatek/custom/$(PROJECT)/kernel/dct/dct/codegen.dws $(DEAL_STDOUT_DRVGEN) && \
...
#	$(hide) perl $(OTA_SCATTER_GENERATOR) $(SCATTER_FILE) $(OTA_SCATTER_FILE)
	 ...
	 $(hide) cd $(LK_WD) && \
	  ...
	  cd $(MKTOPDIR) && \
          ...
          $(if $(strip $(ACTION)),:,$(call copytoout,$(LOGO_IMAGE),$(LOGDIR)/$(PROJECT))) && \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) || \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG))
  endif
else
