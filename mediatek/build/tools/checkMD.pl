...
if (($ENV{MTK_ENABLE_MD1} eq "yes") && &isMismatchMDVer($MD_IMG_MODE,$mode)) {
	$modeErrCnt++;
...
if (($ENV{MTK_ENABLE_MD2} eq "yes") && &isMismatchMDVer($MD_IMG_MODE_2ND,$mode_2nd)) {
	$modeErrCnt++;
