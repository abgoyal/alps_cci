...
$(2): $$(MTK_CGEN_MAKEFILE) $$(wildcard $(1)/*.h)
ifeq ($(MTK_DEPENDENCY_AUTO_CHECK), true)
	-@echo [Update] $$@: $$?
endif
	$(hide) if [ -f $$@ ]; then rm -f $$@; fi;
...
ifeq ($(MTK_DEPENDENCY_AUTO_CHECK), true)
	...
	$(hide) if [ -f $(MTK_CGEN_AP_Editor2_Temp_DB) ]; then rm -f $(MTK_CGEN_AP_Editor2_Temp_DB); fi
	$(hide) if [ -f $(MTK_CGEN_AP_Editor_DB_Enum_File) ]; then rm -f $(MTK_CGEN_AP_Editor_DB_Enum_File); fi
...
ifeq ($(MTK_DEPENDENCY_AUTO_CHECK), true)
	...
	$(hide) if [ -f $(MTK_BTCODEGEN_dbfolder)/BPGUInfo ]; then rm -f $(MTK_BTCODEGEN_dbfolder)/BPGUInfo; fi
	$(hide) if [ -f $(MTK_BTCODEGEN_dbfolder)/enumFile ]; then rm -f $(MTK_BTCODEGEN_dbfolder)/enumFile; fi
