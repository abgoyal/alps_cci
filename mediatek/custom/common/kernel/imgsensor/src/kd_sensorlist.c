...
static atomic_t g_CamHWOpening; // use for checking cam poweron of nots
...
MINT32 i = 0;
    ...
    for ( i = (KDIMGSENSOR_MAX_INVOKE_DRIVERS-1) ; i >= KDIMGSENSOR_INVOKE_DRIVER_0 ; i-- ) {
        ...
        if ( g_bEnableDriver[i] && g_pInvokeSensorFunc[i] ) {
			...
			atomic_set(&g_CamHWOpening, 1);	// for kdCheckSensorPowerOn
		...
		PK_XLOG_INFO("kd_MultiSensorOpen: switch I2C BUS1\n");
            ...
            if ( ERROR_NONE != ret ) {
				...
				atomic_set(&g_CamHWOpening, 0);	// for kdCheckSensorPowerOns
...
u32 i = 0;
	...
	if(On)
		...
		atomic_set(&g_CamHWOpening, 0); // for kdCheckSensorPowerOn
    for ( i = KDIMGSENSOR_INVOKE_DRIVER_0 ; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ ) {
...
{	
	PK_DBG("kdCheckSensorPowerOn, g_CamHWOpening = %d\n", atomic_read(&g_CamHWOpening));
...
inline static int adopt_CAMERA_HW_Open(void)
{
    ...
    if (err == 0 ) {
        ...
        PK_DBG("adopt_CAMERA_HW_Open, g_CamHWOpend = %d\n", atomic_read(&g_CamHWOpend));
