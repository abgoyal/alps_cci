...
extern int ft6x06_i2c_Read(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen);
...
int fts_ctpm_fw_upgrade_with_bin_file(struct i2c_client *client,char *firmware_name,char* vid,char* firmware)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	char src_vid=0;
	...
	//Check version
	printk("get source version:%x,%x\n",pbt_buf[fwsize-1],pbt_buf[fwsize-2]);
	printk("get local version:%x,%x\n",*vid,*firmware);
	src_vid=pbt_buf[fwsize-1];
	src_fvm=pbt_buf[fwsize-2];

	if((src_vid!=*vid) || (*firmware == src_fvm) ||(*firmware > src_fvm))
	{
		printk("[shaohui]not fit the vendor or old than now!\n");
		return -EIO;
	}
...
int ft5x0x_autoupgrade(struct i2c_client *client,char* vid,char* firmware)
{
	//shaohui add for Auto check
	struct file *pfile = NULL;
	char *filepath = "/mnt/sdcard/FC_CTP_FW.bin";
	int ret=-1;	
	...
	if (IS_ERR(pfile)) {
		...
		printk("[shaohui]Open file OK!\n");
//step 2 check	version
	


	disable_irq(client->irq);
	ret = fts_ctpm_fw_upgrade_with_bin_file(client, filepath,vid,firmware);
