...
static u8 MSG21XX_update_bin[]=
{
#include "MSG21XX_FW_BIN.i"
};
#endif
...
static int firmware_version(u16 *major, u16 *minor)
{
	unsigned char tx_ver_buf[3] = {0x53, 0x00, 0x2A};
	unsigned char rx_ver_buf[4] = {0x00};
	...
	if (i2c_master_send(i2c_client, tx_ver_buf, sizeof(tx_ver_buf)) < 0)
	{
		major = minor = 0xFFFF;
		return -1;
	}
	if (i2c_master_recv(i2c_client, &rx_ver_buf, sizeof(rx_ver_buf)) < 0)
	{	
		major = minor = 0xFFFF;
		return -1;
	}

	*major = (rx_ver_buf[1]<<8 | rx_ver_buf[0]);
	*minor = (rx_ver_buf[3]<<8 | rx_ver_buf[2]);

	return 0;
}

static int getMSG21XXFWVersion()
{
	u16 ver_major, ver_minor;
	
	if (firmware_version(&ver_major, &ver_minor) < 0)
	{
	    TPD_DMESG("ERROR:[%s] read firmware version failed\n", __FUNCTION__);
		return -1;
	}        
    curr_ic_major = ver_major;
    curr_ic_minor = ver_minor;
    return 0;
}

#ifdef ENABLE_AUTO_UPDATE
static int fwAutoUpdateThread(void *unused)
{
	//set prioprity to make touch available before lock screen
	struct sched_param param = { .sched_priority = RTPM_PRIO_TPD };
	sched_setscheduler(current, SCHED_RR, &param);
	
	TPD_DMESG("[fwAutoUpdateThread] start");
    firmware_update_store(NULL, NULL, NULL, 0);	
	TPD_DMESG("[fwAutoUpdateThread] end");

    return 0;
}

void firmware_auto_update()
{
    u16 update_bin_major=0;
    u16 update_bin_minor=0;
	int force_update_flag = 0;
	int fw_size_kbytes = 0;
	...
	else if(curr_ic_type == CTP_ID_MSG21XX)
	{
		...
		fw_size_kbytes = 94; //94kbyes
	}

	if (getMSG21XXFWVersion() < 0)
		force_update_flag = 1;

	TPD_DMESG("[firmware_auto_update] bin_major=0x%04x \nbin_minor=0x%04x \n", update_bin_major, update_bin_minor);
	TPD_DMESG("[firmware_auto_update] force_update_flag=%d \n", force_update_flag);

	// case 1: launch upgrade process when i2c transfer error detected
	// case 2: do upgrade process when the new version detected
    if ((1 == force_update_flag) || (update_bin_major > curr_ic_major) ||\
		((update_bin_major == curr_ic_major) && (update_bin_minor > curr_ic_minor)))
    {
        int i = 0;
    	for (i = 0; i < 33; i++)
    	{
        ...
        kthread_run(fwAutoUpdateThread, 0, "fw_auto_update");
    }
   
}
#endif

static ssize_t firmware_getversion_show(struct device *dev,
										struct device_attribute *attr, const char *buf, size_t size)
{
	u16 ver_major, ver_minor;
	...
	if (firmware_version(&ver_major, &ver_minor) < 0)
	{
		return sprintf(buf, "[FW] can't get firmware version\n");
	}

	//TPD_DMESG("Version Major:0x%04x Version Minor:0x%04x\n", ver_major, ver_minor);
	return sprintf(buf, "Version Major:0x%04x\nVersion Minor:0x%04x\n", ver_major, ver_minor);
}
...
static DEVICE_ATTR(get_version, 0664, firmware_getversion_show, NULL);
...
void msg2133_init_class()
{
	...
	// get_version /sys/class/mtk-tpd/device/get_version
	if(device_create_file(firmware_cmd_dev, &dev_attr_get_version) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_get_version.attr.name);
...
static int msg2133_enable_ps(int enable)
{
	...
	mutex_lock(&ps_sensor.ps_mutex);
	TPD_DMESG("msg2133 do enable: %d, current state: %d\n", enable, ps_sensor.open);
	...
	if(enable == 1)
	{
		if(ps_sensor.open == 0)
		{
			...
			TPD_DMESG("msg2133 do enable: %d, current state: %d\n", enable, ps_sensor.open);
			if (i2c_master_send(i2c_client, ps_store_data, 4) < 0)
			{
				TPD_DMESG("ERROR: MSG2133 enable psensor failed\n");
			}
			ps_sensor.open = 1;
		}
	}
	...
	{	
		if(ps_sensor.open == 1)
		{
			...
			if (i2c_master_send(i2c_client, &ps_store_data[0], 4) < 0);
			{
				TPD_DMESG("ERROR: MSG2133 enable psensor failed\n");
			}
			ps_sensor.open = 0;			
	...
	mutex_unlock(&ps_sensor.ps_mutex);
	return 0;
}
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{				
				...
				if(value)
				{
					TPD_DMESG("[%s] Sensor enable:value=%d\n",__FUNCTION__, value);
					wake_lock(&ps_sensor.wk_lock);
					if(err = msg2133_enable_ps(1))
					{
						TPD_DEBUG("enable ps fail: %d\n", err); 
						return -1;
					}
					ps_sensor.open = 1;
				}
				...
				{
					TPD_DMESG("[%s] Sensor enable:value=%d\n",__FUNCTION__, value);
					wake_unlock(&ps_sensor.wk_lock);
					if(err = msg2133_enable_ps(0))
					{
						TPD_DEBUG("disable ps fail: %d\n", err); 
						return -1;
					}
					ps_sensor.open = 0;
				}
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				sensor_data->values[0] = ps_sensor.date;
...
static void tpd_initialize_ps_sensor_function()
{
 ...
 static int tpd_touchinfo(TouchScreenInfo_t *touchData)
 {
    ...
    if( ( Checksum == val[7] ) && ( val[0] == 0x52 ) ) //check the checksum  of packet
    {
		...
		{
			TPD_DMESG("[%s] close panel\n",__FUNCTION__);
			if((ps_sensor.open == 1) && (ps_sensor.date == 1))
			{
				ps_sensor.date = 0;
			}
			return 0;
		...
		{
			TPD_DMESG("[%s] open panel\n",__FUNCTION__);
			if(/*(g_bPsSensorOpen == 1) && */(ps_sensor.date == 0))
			{
				ps_sensor.date = 1;
			}
			return 0;
    ...
    {
		#if 0    
        TPD_DEBUG(KERN_ERR "Packet error 0x%x, 0x%x, 0x%x", val[0], val[1], val[2]);
        TPD_DEBUG(KERN_ERR "             0x%x, 0x%x, 0x%x", val[3], val[4], val[5]);
        TPD_DEBUG(KERN_ERR "             0x%x, 0x%x, 0x%x", val[6], val[7], Checksum);
		#endif		
 ...
 static int touch_event_handler(void *unused)
 {
	...
	if (tpd_touchinfo(&touchData)) 
	{
		...
		if( touchData.nTouchKeyMode )
		{
			switch (touchData.nTouchKeyCode)
			{
				case 1:
					...
					touchData.Point[0].X = tpd_keys_dim_local[0][0];
					touchData.Point[0].Y = tpd_keys_dim_local[0][1];
					break;

				case 2:
					touchData.nFingerNum = 1;
					//touchkeycode = KEY_HOME;
					touchData.Point[0].X = tpd_keys_dim_local[1][0];
					touchData.Point[0].Y = tpd_keys_dim_local[1][1];
					break;

				case 4:
					touchData.nFingerNum = 1;
					//touchkeycode = KEY_BACK;
					touchData.Point[0].X = tpd_keys_dim_local[2][0];
					touchData.Point[0].Y = tpd_keys_dim_local[2][1];
					break;
		...
		pre_touch = (u8)touchData.nFingerNum;
...
//power on, need confirm with SA
	...
	if (CTP_ID_UNKNOW == curr_ic_type)
	{
		TPD_DMESG("ERROR: Get MSG21XX chip type failed");
		return -1;
	}
	msg2133_reset();
	tpd_load_status = 1;
...
#ifdef TPD_PROXIMITY
	mutex_init(&ps_sensor.ps_mutex);
	wake_lock_init(&ps_sensor.wk_lock, WAKE_LOCK_SUSPEND, "tp_ps_wlock");
...
//#endif 
...
#ifdef TPD_PROXIMITY
	if(ps_sensor.open == 0 && (!ps_sensor.suspend))
	{
		TPD_DMESG("Resume in calling, no need to resume\n");
		return 0;
	}
	ps_sensor.suspend = 0;
#endif
 ...
 static void tpd_suspend( struct early_suspend *h )
 {
	...
	if(ps_sensor.date == 0)
	{
		TPD_DMESG("Suspend in calling, no need to suspend\n");
		return 0;
	}
	ps_sensor.suspend = 1;
#endif
