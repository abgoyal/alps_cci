...
static bltable_t bltab;

static int dump_buffer_size = 0;
static int dump_data_size = 0;
static int dump_overrun = 0;
static int dump_overrun_size = 0;
static int dump_seq_no = 0;
static void *dump_buffer = NULL;

#define OPFLAG_OVERWRITE	0x1
static unsigned int options_flag = 0;
...
LIST_HEAD(met_devlink);

int met_devlink_add(struct metdevice* met)
{
	struct metdevice *c;

	list_for_each_entry(c, &met_devlink, exlist) {
		if (!strcmp(c->name, met->name)) {
			return -EEXIST;
		}
	}
	INIT_LIST_HEAD(&met->exlist);
	list_add(&met->exlist, &met_devlink);
	return 0;
}
...
EXPORT_SYMBOL(met_devlink_add);
EXPORT_SYMBOL(met_devlink_del);
EXPORT_SYMBOL(met_devlink_register_all);
EXPORT_SYMBOL(met_devlink_deregister_all);
...
static long met_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case MTAG_CMD_DUMP:
			...
			if (ret) return -EFAULT;
			ret =  met_tag_dump(pUserCmd->class_id, pUserCmd->tname, pUserCmd->data, pUserCmd->size);
		break;
...
static ssize_t met_write(struct file *filp, const char __user *buf,
...
static ssize_t met_read(struct file *filp, char __user *buf,
                          size_t count, loff_t *f_pos)
{
...
static ssize_t enable_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t n)
{
	int value;
...
static struct kobject *kobj_tag = NULL;
...
static ssize_t dump_buffer_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t n)
{
	int ret, value;
...
static struct kobj_attribute dump_buffer_attr = __ATTR(dump_buffer_kb, 0644, dump_buffer_show, dump_buffer_store);
...
static ssize_t options_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t n)
{
	if ((n == 0) || (buf == NULL)) {
		return -EINVAL;
	}
