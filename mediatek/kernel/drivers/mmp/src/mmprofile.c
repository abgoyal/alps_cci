...
#define MMProfileDumpBlockSize 1024
...
static void MMProfileInitBuffer(void)
{
        ...
        (MMProfileGlobals.meta_buffer_size != MMProfileGlobals.new_meta_buffer_size))
    {
        ...
        printk("[mmprofile_buffer_init] pMMProfileRingBuffer=0x%08x\n", (unsigned int)pMMProfileRingBuffer);
        ...
        printk("[mmprofile_buffer_init] pMMProfileMetaBuffer=0x%08x\n", (unsigned int)pMMProfileMetaBuffer);
        ...
        if ((!pMMProfileRingBuffer)||(!pMMProfileMetaBuffer))
        {
            ...
            printk("[mmprofile_buffer_init] Cannot allocate buffer.\n");
            return;
        }
...
{ 
    if (!MMProfileGlobals.enable)
        return;
    printk("[mmprofile_start] Start mmprofile: %d\n", start);
    ...
    printk("[mmprofile_start] MMProfileStart=%d\n", MMProfileGlobals.start);
}
...
{
    printk("[mmprofile_enable] Enable mmprofile: %d\n", enable);
    if (enable)
...
static void system_time(unsigned int *low, unsigned int *high)
{
	...
	*high = (int32_t)((temp>>32)&0xffffffff);
	//printk("system_time,0x%08x,0x%08x\n", *high, *low);
}
...
static ssize_t mmprofile_dbgfs_start_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    char str[32];
    int r;
    printk("[mmprofile] DbgFS: Read start. start=%d\n", MMProfileGlobals.start);
...
static ssize_t mmprofile_dbgfs_start_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
    ...
    if ((str&0xFF) == 0x30)
        ...
        start = 1;
    printk("[mmprofile] DbgFS: Write start. start=%d\n", start);
...
static ssize_t mmprofile_dbgfs_enable_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    char str[32];
    int r;
    printk("[mmprofile] DbgFS: Read enable. enable=%d\n", MMProfileGlobals.enable);
...
static ssize_t mmprofile_dbgfs_enable_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
    ...
    if ((str&0xFF) == 0x30)
        ...
        enable = 1;
    printk("[mmprofile] DbgFS: Write enable. enable=%d\n", enable);
...
static ssize_t mmprofile_dbgfs_buffer_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    ...
    if (!bMMProfileInitBuffer)
        return -EFAULT;
    //printk("[mmprofile] DbgFS: Read buffer. size=%d ppos=%d\n", size, (int)(*ppos));
    if (*ppos == 0)
    {
    ...
    while (size > 0)
    {
        ...
        copy_to_user(buf + total_copy, (void*)Addr, copy_size);
        *ppos += copy_size;
