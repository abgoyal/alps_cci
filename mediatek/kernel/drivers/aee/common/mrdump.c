...
static int __init aee_kdump_hw_init(void)
{
	...
	mrdump_machdesc.output_device = MRDUMP_DEV_SDCARD;
	mrdump_machdesc.nr_cpus = NR_CPUS;
	mrdump_machdesc.page_offset = (void *)PAGE_OFFSET;
	mrdump_machdesc.high_memory = (void *)high_memory;

        mrdump_machdesc.vmalloc_start = (void *)VMALLOC_START;
        mrdump_machdesc.vmalloc_end = (void *)VMALLOC_END;

        mrdump_machdesc.modules_start = (void *)MODULES_VADDR;
        mrdump_machdesc.modules_end = (void *)MODULES_END;

	mrdump_machdesc.phys_offset = (void *)PHYS_OFFSET;
	mrdump_machdesc.master_page_table = (void *)&swapper_pg_dir;
	mrdump_machdesc.crash_record = (void *) __pa(&mrdump_crash_record);
	...
	aee_rr_rec_kdump_params((void *)__pa(&mrdump_machdesc));
...
static int param_set_mrdump_device(const char *val, const struct kernel_param *kp)
{
	...
	else if (strcmp(strval, "sdcard") == 0) {
		eval = MRDUMP_DEV_SDCARD;
	}
	else if (strcmp(strval, "emmc") == 0) {
...
{
	char *dev;
	switch (mrdump_machdesc.output_device) {
...
module_param_named(enable, mrdump_enable, bool, S_IRUGO | S_IWUSR);
...
param_check_int(device, &(mrdump_machdesc.output_device));
module_param_cb(device, &param_ops_mrdump_device, &mrdump_machdesc.output_device, S_IRUGO | S_IWUSR);
