/*
** $Id: //Department/DaVinci/BRANCHES/MT6628_WIFI_DRIVER_V2_1/os/linux/gl_init.c#1 $
*/
...
** $Log: gl_init.c $
 *
 * 05 31 2012 yuche.tsai
 * NULL
 * Bug fix, dissolve group instead of switching OP mode only during formation error handling.
 *
 * 05 31 2012 yuche.tsai
 * NULL
 * Update for MOTO argonmini bug fix.
 *
 * 01 20 2012 yuche.tsai
 * NULL
 * Fix P2P GC TX deauth issue.
 *
...
static BOOLEAN g_fgP2pFuncIsApMode = FALSE;
#endif
...
#if CFG_6572_SPM_WORKAROUND
        if (prGlueInfo->prAdapter->rP2pFuncLkr.prP2pFuncIsApMode(prGlueInfo->prAdapter->rWifiVar.prP2pFsmInfo)) {
            DBGLOG(INIT, INFO, ("Hold AP wakelock!\n"));
            g_fgP2pFuncIsApMode = TRUE;
...
#if CFG_6572_SPM_WORKAROUND
        if (g_fgP2pFuncIsApMode) {
            DBGLOG(INIT, INFO, ("Release AP wakelock!\n"));
            KAL_WAKE_UNLOCK(prGlueInfo->prAdapter, &prGlueInfo->prAdapter->rApWakeLock);
            g_fgP2pFuncIsApMode = FALSE;
        }
#endif
    ...
    )
{
    ...
    if(prGlueInfo->prAdapter->fgIsP2PRegistered) {
        ...
        prFuncLkr->prP2pNetRegister(prGlueInfo);
    }
    ...
    )
{
    ...
    if(prGlueInfo->prAdapter->fgIsP2PRegistered) {
        P_P2P_FUNCTION_LINKER prFuncLkr = &(prGlueInfo->prAdapter->rP2pFuncLkr);
        prFuncLkr->prP2pNetUnregister(prGlueInfo);
    }
    ...
    )
{
    ...
    DBGLOG(INIT, INFO, ("unregisterdev(0x%p)\n", prDev));
    //rtnl_unlock();
    ...
    )
{
    ...
    prGlueInfo = (P_GLUE_INFO_T) netdev_priv(prDev);
    printk("<wlan> allocate prGlueInfo = 0x%p\n", prGlueInfo); //samp
...
bailout:
        ...
        /*if p2p module is inserted, start it*/
