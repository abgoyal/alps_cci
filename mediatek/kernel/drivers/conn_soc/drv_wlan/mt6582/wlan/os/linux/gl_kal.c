/*
** $Id: //Department/DaVinci/BRANCHES/MT6628_WIFI_DRIVER_V2_1/os/linux/gl_kal.c#2 $
*/
...
//    prGlueInfo->u4TimeoutFlag = 0;
    ...
    /* <9> Block and wait for event or timeout, current the timeout is two secs */

//    if (wait_for_completion_interruptible_timeout(&prGlueInfo->rPendComp, 2 * KAL_HZ)) {
    ...
    )
{
    ...
    while (prQueueEntry) {
        ...
        if (prCmdInfo->eCmdType == COMMAND_TYPE_MANAGEMENT_FRAME &&
                prCmdInfo->eNetworkType == eNetworkTypeIdx) {
            P_MSDU_INFO_T prMsduInfo = (P_MSDU_INFO_T)prCmdInfo->prPacket;
