...
extern unsigned char mtkfb_debug_flag;
...
static int cnt=3;
static int regupd_timeout_count = 0;
...
*/
 ...
 //       DISP_WaitForLCDNotBusy();
    ...
    wait_event_ret = wait_event_interruptible_timeout(reg_update_wq, PanDispSettingApplied, HZ/10);
    if ((wait_event_ret == 0) && (PanDispSettingApplied == 0) && (!is_early_suspended))
    {
        printk("[FB Driver] PanDispSettingApplied Timeout\n");
        regupd_timeout_count ++;
        DISP_StartConfigUpdate();
    }
    else
    {
        regupd_timeout_count = 0;
    }
...
static int mtkfb_set_overlay_layer(struct fb_info *info, struct fb_overlay_layer* layerInfo)
{
    ...
    */
    ...
    case MTK_FB_FORMAT_YUV422:
        cached_layer_config[id].fmt = OVL_INPUT_FORMAT_UYVY;
...
static int mtkfb_capture_framebuffer(struct fb_info *info, unsigned int pvbuf)
{
    ...
    */
    ...
    {
        if (atomic_read(&capture_ui_layer_only) || is_early_suspended)
        { 
        		unsigned int fbv;
            ...
            }
            {
                ...
                if (is_early_suspended)
                {
                    for(i = 0;i < h_yres; i++)
                    {
                        memset((void *)(pvbuf + i * w_xres * pixel_bpp), 0, w_xres * pixel_bpp);
                    }
                }
                else
                {
...
static int mtkfb_ioctl(struct file *file, struct fb_info *info, unsigned int cmd, unsigned long arg)
{
    ...
    case MTKFB_SET_VIDEO_LAYERS:
    {
        ...
        // mtkfb force stopping flag
        if (mtkfb_debug_flag == 1)
        {
            return 0;
        }
	...
	case MTKFB_UNREGISTER_OVERLAYBUFFER:
    {
		...
		else
		{
			...
			for (i = 0; i < VIDEO_LAYER_COUNT; i++)
			{
				...
				msleep(20);
				while (((cached_layer_config[i].isDirty == TRUE) && (cached_layer_config[i].layer_en == TRUE) && (cached_layer_config[i].vaddr == overlay_bufferVA)) ||
					((captured_layer_config[i].isDirty == TRUE) && (captured_layer_config[i].layer_en == TRUE) && (captured_layer_config[i].vaddr == overlay_bufferVA)) ||
					((realtime_layer_config[i].layer_en == TRUE) && (realtime_layer_config[i].vaddr == overlay_bufferVA)))
				{
					msleep(20);
					retryCount ++;
					if (retryCount > 50)
					{
						printk("[mtkfb_ioctl] MTKFB_UNREGISTER_OVERLAYBUFFER : wait overlay disable failed\n");
						break;
					}
				}
			}
