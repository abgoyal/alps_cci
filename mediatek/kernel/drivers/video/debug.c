...
unsigned char mtkfb_debug_flag=0;
...
//            DPI1_DumpRegisters();
    ...
    else if (0 == strncmp(opt, "fbx", 3))
    {
        mtkfb_debug_flag = mtkfb_debug_flag ^ 1;
        DISP_LOG_PRINT(ANDROID_LOG_INFO, "DBG", "mtkfb_debug_flag: %d\n", mtkfb_debug_flag);
    }
    else if (0 == strncmp(opt, "fb_layer:", 9))
    {
        if (0 == strncmp(opt + 9, "on:", 3))
        {
            char *p = (char *)opt + 12;
            unsigned int layer = (unsigned int) simple_strtoul(p, &p, 10);

            mutex_lock(&OverlaySettingMutex);
            LCD_CHECK_RET(LCD_LayerEnable(layer, 1));
            atomic_set(&OverlaySettingDirtyFlag, 1);
            atomic_set(&OverlaySettingApplied, 0);
            mutex_unlock(&OverlaySettingMutex);

            DISP_LOG_PRINT(ANDROID_LOG_INFO, "DBG", "fb_layer: %d is on\n", layer);
        }
        else if (0 == strncmp(opt + 9, "off:", 4))
        {
            char *p = (char *)opt + 13;
            unsigned int layer = (unsigned int) simple_strtoul(p, &p, 10);
            
            mutex_lock(&OverlaySettingMutex);
            LCD_CHECK_RET(LCD_LayerEnable(layer, 0));
            atomic_set(&OverlaySettingDirtyFlag, 1);
            atomic_set(&OverlaySettingApplied, 0);
            mutex_unlock(&OverlaySettingMutex);

            DISP_LOG_PRINT(ANDROID_LOG_INFO, "DBG", "fb_layer: %d is off\n", layer);
        }
    }
    else
    {
